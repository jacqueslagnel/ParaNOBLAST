...
then
	...
	echo -en "\nUSAGE:\n"
	echo "$0 <input fasta file full path> <nr|nt|custom (full path)> <blast program: blastx|blastp|blastn> <E-value (format e.g.: 0.01, 1e-10)> <nb hit to keep> <output full path (/mypath/myblastoutput)>"
...
then
11	echo "ERROR: give fasta and output files with full path..."
	exit 0
fi
...
#----------------- blast databases ------------------------------------
if [ "$blastdb" != "nr" ] && [ "$blastdb" != "nt" ] && [ ! -f "$blastdb" ]
then
	echo "blast database must be nr|nt|valid custom db"
	exit 0
fi
...
then
		if [ ! -s "${blastdb}.dbs" ]
		then
       			echo "blast database must be formated"
       			exit 
		fi
		blsdb="${blastdb}.*.phr"
		blsdb_size=`grep '^[0-9]' ${blastdb}.dbs|sed '1q;d'`
		blsdb_nbseq=`grep '^[0-9]' ${blastdb}.dbs|sed '2q;d'`
fi
